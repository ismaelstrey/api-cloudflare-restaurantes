...
export function createUserRoutes(db: DatabaseClient) {
	 ...
	 * @access Public
	 */
	userRoutes.post('/register', zValidator('json', registerUserSchema), async (c) => await userController.register(c));
	 ...
	 * @access Public
	 */
	userRoutes.post('/login', zValidator('json', loginUserSchema), async (c) => await userController.login(c));
	 ...
	 * @access Private
	 */
	userRoutes.get('/profile', authMiddleware(), async (c) => await userController.getProfile(c));
	 ...
	 * @access Private
	 */
	userRoutes.put('/profile', authMiddleware(), zValidator('json', updateUserSchema), async (c) => {
	 ...
	 * @access Private
	 */
	userRoutes.patch('/profile/password', authMiddleware(), zValidator('json', changePasswordSchema), async (c) => {
		// Pega o ID do usuário autenticado
		const userId = c.get('user')?.userId;
		if (!userId) {
			return c.json({ error: 'Usuário não autenticado' }, 401);
	 ...
	 */
	userRoutes.get('/admin', async (c) => await userController.list(c));
	 ...
	 * @access Private (Admin)
	 */
	userRoutes.get('/admin/:id', zValidator('param', idParamSchema), async (c) => await userController.getById(c));
	 ...
	 * @access Private (Admin)
	 */
	userRoutes.patch('/admin/:id/deactivate', zValidator('param', idParamSchema), async (c) => await userController.deactivate(c));
	 ...
	 * @access Private (Admin)
	 */
	userRoutes.patch('/admin/:id/activate', zValidator('param', idParamSchema), async (c) => await userController.activate(c));
