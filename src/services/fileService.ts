...
export class FileService {
	 ...
	 */
	async uploadFile(file: File, userId: string, options: FileUploadOptions): Promise<UploadedFile> {
		try {
			// Validar arquivo
			this.validateFile(file, options);

			// Gerar chave única
			const fileKey = options.generateUniqueKey !== false ? this.generateFileKey(file.name, userId) : `uploads/${userId}/${file.name}`;
	 ...
	 */
	async uploadMultipleFiles(files: File[], userId: string, options: FileUploadOptions): Promise<UploadedFile[]> {
	...
	async getStorageStats(userId?: string): Promise<{
		...
		try {
			...
			const typeDistribution = arquivos.reduce(
				(acc, arquivo) => {
