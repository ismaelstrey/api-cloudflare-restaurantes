...
export const registerUserSchema = z.object({
  ...
  }).email('Email deve ter um formato válido').max(255, 'Email deve ter no máximo 255 caracteres'),
...
export const updateUserSchema = z.object({
  ...
  senha: z.string().min(8, 'Senha deve ter pelo menos 8 caracteres')
    .regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/, 'Senha deve conter pelo menos uma letra maiúscula, uma minúscula e um número').optional(),
